...
LEGACY_OUT_DIR = ROOT / "docs/research/rmab_vm_outputs/baseline_setting_suite_v1"
BUNDLE_OUT_DIR = REPO_ROOT / "Experiments_Results/baseline_setting_suite_v1"
...
def main() -> None:
    ...
    bar_gs = gs[0, 1].subgridspec(2, 1, hspace=0.82)
    ...
    ax_curve.text(0.0, 1.075, "(a)", transform=ax_curve.transAxes, fontsize=15, fontweight="bold", va="bottom")
    ax_curve.set_xlabel("Round", fontsize=17)
    ax_curve.set_ylabel("Average reward across rounds ($/MWh)", fontsize=15)
    ...
    for ax, round_value in zip(bar_axes, ROUNDS):
        ...
        ax.set_title(f"Round {round_value}", loc="left", fontweight="bold", pad=6, fontsize=16)
        ax.set_xticks(list(xs))
        ax.set_xticklabels(data["display_name"].tolist(), rotation=52, ha="right", rotation_mode="anchor", fontsize=13.0)
        ax.set_ylabel("Cumulative average reward\n(% of Oracle)", fontsize=13.0)
        ...
        for idx, (bar, value, err) in enumerate(zip(bar_artists, values, errors)):
            if idx != best_idx:
                continue
            ax.text(
    ...
    ax_bar_100.text(0.0, 1.20, "(b)", transform=ax_bar_100.transAxes, fontsize=15, fontweight="bold", va="bottom")

    bar_legend_roles = ["Baselines", "TW", "Refined TW"]
    bar_handles = [
    ...
    fig.subplots_adjust(left=0.08, right=0.99, top=0.84, bottom=0.28)
    fig.savefig(OUT_DIR / "fig_baseline_setting_learning_and_round_bar.png", dpi=300, bbox_inches="tight", pad_inches=0.14)
    fig.savefig(OUT_DIR / "fig_baseline_setting_learning_and_round_bar.pdf", bbox_inches="tight", pad_inches=0.14)
    plt.close(fig)
